...
ggplot(BRA_complete, aes(x = Date, y = Temperature_C)) +
  geom_line() +
  geom_smooth(method = "lm")
  labs(x = "", y = expression("Temperature"))
```
...
BRA_ts_rainfall <- ts(BRA_complete$Rainfall, start = c(1901,1), frequency = 12)

BRA_ts_temperature <- ts(BRA_complete$Temperature_C, start = c(1901,1), frequency = 12)
...
rainfall_decomposed <- stl(BRA_ts_rainfall, s.window = "periodic")
temperature_decomposed <- stl(BRA_ts_temperature, s.window = "periodic")
...
BR_Rain_Components <- as.data.frame(rainfall_decomposed$time.series[,1:3])

BR_Temperature_Components <- as.data.frame(temperature_decomposed$time.series[,1:3])


BR_Rain_Components <- mutate()
