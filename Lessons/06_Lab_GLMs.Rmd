...
> Answer: 
The null hypothesis is that the mean values are not below the threshold for good AQI index.
The alternative hypothesis is that the values are below the threshold for the good AQI index.
...
#now run t-test to see if the mean is different from a specific value.
...
```{r}
#Exercise 2:
#adding the "fill" argument instead of "color"; and set year as a factor
ggplot(EPAair, aes(x = Ozone, fill = as.factor(Year))) +
  geom_density(alpha = 0.5) +
  labs(Title = "hi",
       #fill let's you change the legend!
       fill = "Year") +
  scale_fill_manual(values=c("red", "green"))

ggplot(EPAair, aes(y = Ozone, x = as.factor(Year))) +
  geom_boxplot() +
  #this code lets you grab data from the data
  stat_summary(geom = "text", fun.y = max, vjust = -1, size = 4,
               label = c("a", "b"), color = "red")
  
#this code creates a violin plot
ggplot(EPAair, aes(y = Ozone, x = as.factor(Year))) +
...
ggplot(EPAair, aes(x=Ozone, y = PM2.5)) + 
  ...
  labs(title = "Does Ozone Explain PM2.5?")
...
epa_cor <- cor(EPAair %>% select(SITE_LATITUDE:PM2.5) %>% drop_na())
corrplot(epa_cor, method = "ellipse")
...
epa.lm2 <- lm(data = EPAair, PM2.5 ~ Ozone + Year + Month + SITE_LATITUDE + SITE_LONGITUDE)
summary(epa.lm2)

#Run stepwise reduction function
step(epa.lm2)
