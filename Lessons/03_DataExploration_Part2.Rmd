...
Visualize count data for categorical variables. Bar part is useful for categorical variables and counts.
...
```{r, fig.height = 3, fig.width = 4}
#creates a histrogram of the charage mean values
...
ggplot(USGS.flow.data) +
  ...
  #lty is line type for dashed line
  geom_freqpoly(aes(x = gage.height.max), bins = 50,  lty = 2) +
  scale_x_continuous(limits = c(0, 10))

#
ggplot(USGS.flow.data) +
  #color by mean approval, which is categorical
...
```{r, fig.height = 3, fig.width = 4}
#mean gage height values subsetted by approval
ggplot(USGS.flow.data) +
...
Visualize relationships between continuous numerical variables.This is different from looking at distribtuions within variables, which is more or less what we've been doing before this.
