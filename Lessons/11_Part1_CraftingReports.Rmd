...
* R Markdown is a convenient way to produce **aesthetically pleasing reports**.
* R markdown operates through a **text language**, so it can be easy uploaded to github, unlike Microsoft products.
* In a realted sense, it can therefore also be **easily shared**, which creates *greater transparency* in your analysis.
* R Markdown can handle many **different coding languages**, like Python.
  + This can be useful for projects that are more easily accomplished in another language due to the availability of packages.
...
NTL.nutrients.data <- read.csv("../Data/Raw/NTL-LTER_Lake_Nutrients_Raw.csv", stringsAsFactors = TRUE)
head(NTL.nutrients.data)
NTL.nutrients.data$sampledate <- as.Date(NTL.nutrients.data$sampledate, format = "%m/%d/%y")
```
...
```{r}
#summarize for nitrogen
NTL.nutrients.nitrogen <- NTL.nutrients.final %>%
  group_by(lakename) %>%
  summarize(
...
#summarize for phospherous 
NTL.nutrients.phospherous <- NTL.nutrients.final %>%
  group_by(lakename) %>%
  summarize(
...
```{r tables, echo=FALSE}
#nitrogen tables
kable(NTL.nutrients.nitrogen, caption = "Surface Samples: Total Nitrogen")
#phospherous tables
kable(NTL.nutrients.phospherous, caption = "Surface Samples: Total Phospherous")
```
...
```{r nitrogen plot, echo=FALSE, warning=FALSE, fig.align='left', message = FALSE, fig.cap="Total Nitrogen"}
#create the plot for the nitrogen
ggplot(data = NTL.nutrients.final, aes(x = sampledate, y = tn_ug, color = lakename)) +
  geom_point() +
  geom_smooth(se=F) +
  labs(
...
```
```{r r nitrogen plot, echo=FALSE, warning=FALSE, fig.align='left', message=FALSE, fig.cap="Total Phospherous"}
#create for plot for the phospherous
ggplot(data = NTL.nutrients.final, aes(x = sampledate, y = tp_ug, color = lakename)) +
  geom_point() +
  geom_smooth(se=F) +
  labs(
...
In the above plots, I analyzed the nitrogen and phosphorous content of 8 different lakes over time, from 1991 to 2000. These 8 lakes are part of a long term ecological research program (LTER); learning about the phosphorous and nitrogen content for these lakes can tell us a great deal about the overall health of lakes. In circumstances where lakes are affected by human development (either cities or agricultural production), they will often experience nutrient overloading, meaning that they will have more nutrients like phosphorous and nitrogen than they would in a natural state. This can lead to dramatically destabilized conditions in the lakes, like algal blooms that wipe out other wildlife. Looking at the nitrogen plot (figure 1), we can see that for East Long, West Long, and Peter Lake, there appears to be a general trend from 1991 to 1997 of increasing nitrogen content, which then appears to decrease in the years afterwards. The other lakes with sufficient data appear to have substantially more stable nitrogen measurements over time. Meanwhile, turning to the phosphorous plot (figure 2), East Long lake appears to display a similar trend in increasing phosphorous from 1991 to 1997, which then begins to decline. West Long and Peter lake show a slightly increasing trend, but not nearly as substantial. The other lakes do not appear to show a substantial trend. We might conclude that the lakes showing substantial change also had nearby land use changes.

******
With further analysis, several different questions could be answered about these lakes. We could, for example, ask whether or not the trend that is visible is statistically significant, or whether it is largely generated by random processes. This could be accomplished using a time series analysis. We could also ask whether there is a relationship between nitrogen and phosphorous content among the different lakes, which would involve constructing a linear model. With additional data, more robust questions could be asked. For example, with land use data, we could determine what types of changes are associated with the increasing trends. With species data, we could also ask what effect the increased nitrogen and phosphorous load has had.
