...
#use the ampersand for readability, but a comma will also work
NTL.phys.data.Tuesday.1990.1999 <- 
  NTL.phys.data %>%
...
> Answer: 
This wouldn't answer our question since the actual row number is arbitrary.
...
NTL.phys.data.July.Tuesday.1990.1999.2 <- 
  NTL.phys.data %>%
  filter(lakename == "Tuesday Lake" & year4 %in% c(1990:1999)) %>%
  mutate(month = month(sampledate)) %>%
  filter(month == 7)
