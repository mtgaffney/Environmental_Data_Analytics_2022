...
max.withdrawals.mgd <- webpage %>% html_nodes('th~ td+ td , th~ td+ td') %>% html_text()
...
scrape.it <- function(the_pwsid, the_year){
  ...
  pswid <- the_website %>% html_nodes(pswid.tag) %>% html_text()
  ...
  df_withdrawals <- data.frame("Month" = c(1, 5, 9, 2, 6, 10, 3, 7, 11, 4, 8, 12),
    ...
    mutate(Water_System_Name = !!water.system.name,
         ...
         Ownership = !!ownership,
         Date = my(paste(Month,"-",Year)))
  Sys.sleep(1) #uncomment this for bulk scraping
...
ggplot(durham.2015,aes(x=Date,y=Max_Withdrawals_mgd)) + 
  geom_line() + 
  geom_smooth(method="loess",se=FALSE) +
  labs(title = paste("2015 Max Water usage data for Durham"),
       y="Withdrawal (mgd)",
       x="Date")
...
#combine the data
durham.asheville.2015 <- bind_rows(durham.2015, asheville.2015)

ggplot(durham.asheville.2015,aes(x=Date,y=Max_Withdrawals_mgd)) + 
  facet_wrap("Water_System_Name") +
  geom_line() + 
  geom_smooth(method="loess",se=FALSE) +
  labs(title = paste("2015 Max Water usage data for Asheville and Durham"),
       y="Withdrawal (mgd)",
       x="Date") +
...
ashville.2010.2019 <- lapply(X = the_years, 
                             FUN = scrape.it, 
                             the_pwsid = my_pwsid)
asheville.final <- bind_rows(ashville.2010.2019)

#create plot for the data
ggplot(asheville.final,aes(x=Date,y=Max_Withdrawals_mgd)) + 
  geom_line() + 
  geom_smooth(method="loess",se=FALSE) +
  labs(title = paste("2010-2019 Max Water usage data for Asheville"),
       y="Withdrawal (mgd)",
       x="Date")
```

>Question: Just by looking at the plot (i.e. not running statistics), does Asheville have a trend in water usage over time?
It seems clear that there is an upward trend in maximum water withdrawal for Asheville after 2015.