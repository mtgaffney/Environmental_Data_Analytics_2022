...
Neon <- read.csv("./data/Processed/NTL-LTER_Lake_Chemistry_Nutrients_PeterPaul_Processed.csv", stringsAsFactors = TRUE)


#2 Check dates for both datasets
class(PeterPaul$sampledate)
class(Neon$sampledate)
#both read as factors; transform both to dates
PeterPaul$sampledate <- as.Date(PeterPaul$sampledate, format = "%Y-%m-%d")
Neon$sampledate <- as.Date(Neon$sampledate, format = "%Y-%m-%d")
...
plot_grid(PeterPaul_Temp, PeterPaul_TP, PeterPaul_TN, my.legend, 
          ncol = 1, align = "vh", axis = "bt", rel_heights = c(1.25, 1))

#plot_grid(PeterPaul_Temp, PeterPaul_Temp, PeterPaul_TN, nrow = 2, align = 'h', rel_heights = c(1.25, 1))
```
